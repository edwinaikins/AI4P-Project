...
async function groomIdeaRawItem(rawItem) {
  ...
  try {
    ...
    if (
      clusterResp &&
      typeof clusterResp === "object" &&
      "cluster_id" in clusterResp
    ) {
      ...
      try {
        ...
        if (p && typeof p.cluster_id !== "undefined")
...
async function fetchSingularityNetIdeas() {
  ...
  const [market, github] = await Promise.all([
  ...
  const needFallback = groomed.some(
    (x) => x.cluster_id === null || typeof x.cluster_id === "undefined"
  );

  if (needFallback) {
    console.log(
      "[PIPE] Some items lacked cluster_id from LLM. Running local KMeans fallback for all items to ensure parity."
    );
...
function scoreFeasibility(ideaText, source) {
  ...
  if (t.includes("research") || t.includes("theory") || t.includes("explor"))
    score -= 20;
  if (
    ...
    score += 20;
  if (t.includes("clinical") || t.includes("health") || t.includes("medical"))
    score -= 5;
...
function categorizeIdea(ideaText) {
  ...
  const categories = [
    [
      "Finance",
      ["finance", "debt", "trading", "portfolio", "defi", "token", "wallet"],
    ],
    ...
    [
      "Infrastructure",
      ["compute", "neural", "inference", "distributed", "ipfs", "protocol"],
    ],
...
export async function runSingularityNetIdeaChecker(newIdeaText) {
  if (typeof newIdeaText !== "string")
    throw new Error("new idea must be a string");
  ...
  console.log(
    "[CHECKER] Grooming the new idea via SAME pipeline (embed + cluster)..."
  );
  ...
  try {
    ...
    console.warn(
      "[CHECKER] runClustering failed for new idea, using local nearest-centroid fallback."
    );
    const embeddings = corpusIdeas.map((c) => c.embedding);
    const N = embeddings.length || 1;
    const k = Math.min(50, Math.max(2, Math.round(Math.sqrt(N))));
    // run KMeans to get centroids
    const km = kmeans(embeddings, Math.min(k, embeddings.length), {
      seed: 42,
      maxIterations: 100,
    });
  ...
  const marketplaceCandidates = deduped.filter((c) =>
    c.idea_id.startsWith("snet:")
  );

  const githubCandidates = deduped.filter((c) =>
    c.idea_id.startsWith("github:")
  );
